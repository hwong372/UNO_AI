...
//this is the deal function that takes in a vector of cards to represent the deck and a vector of Player* to represent each player (5 total)
void deal(vector<Card>& theDeck, vector<Player*>& thePlayers) {
	//deal to every player 
	for (int i = 0; i < thePlayers.size(); i++) {
		//clear each player's hand (at the beginning of the game so no player should have cards at any index in their vector)
		thePlayers[i]->clearHand();

		//each player should get 7 cards 
		for (int j = 0; j < 7; j++) {
...
//this function creates the deck by adding all the cards needed to play 
void createDeck(vector<Card>& theDeck) {
	theDeck.clear();
	//loop through the 4 regular colors (red, green, blue, yellow)
	for (int color = 0; color < 4; color++) {
		//there are 13 numbers (0-12) 
		//each color has 2 of the same number 
		for (int i = 0; i < 13; i++) {
	...
	//there are 8 special cards 
	for (int i = 0; i < 4; i++) {
		//add wild cards (4 total)
		Card c(0, BLACK);
		theDeck.push_back(c);

		//add draw4 cards (4 total)
		Card c2(1, BLACK);
		theDeck.push_back(c2);
	}
}

//this function shuffles the deck so it's fair, duh
...
//this function changes the color of the cards to be held as a string because it's easier that way 
...
//this function prints the game state by stating the turn, the number of cards each player has and the card that is at the top of the pile 
void printGameState(int currentTurn, vector<Player*> thePlayers, Card topCardOfDiscard) {
	...
	if (VERBOSE) {
		cout << "Card on top: " << cardTOString(topCardOfDiscard) << " " << colorToString(topCardOfDiscard.getColorChange()) << "\n";
	}
}
...
void verifyGameStateCards(vector<Player*> thePlayers, vector<Card>& theDeck, vector<Card>& discardPile) {
	...
	if (VERBOSE) {
		cout << "Card Count (should be 112): " << sum << "\n";
	}
}

//this function is for when the current player has to draw a card
...
void main()
{
	...
	thePlayers.push_back(new Player(3, 6)); // the 1 passed in here indicates the ai from the player constructor.
